...
/*foreach($submissions as $submission_val){
	var_dump($submission_val);//userid, id, vpl_id, date submitted, comments, 	
	//echo $submission_val->{'id'};
	echo "<BR><BR>";
}
*/
$submissions_number = $vpl->get_submissions_number();
/*foreach($submissions_number as $sn){
	var_dump($sn);		//userid and number of submissions
	echo "<BR><BR>";
}*/
//var_dump( $submissions_number[321]);
//var_dump($submissions_number);
//Get all information
$listAsso=array();
foreach($list as $le){
$listAsso[$le->{'id'}]=$le;
...
foreach($submissions_number as $sn){
	...
	$c_submission=	$submissions[$sn->userid];
	$y_data=vpl_get_working_periods($vpl, $sn->userid);
	//var_dump($y_data);
	$s=0;
	foreach($y_data as $wperiod){
		$s=$s+$wperiod;
	}
	echo "<td>".$sn->userid."</td> <td> ".$listAsso[$sn->userid]->{'firstname'}." ".$listAsso[$sn->userid]->{'lastname'};
	echo "</td><td>".$sn->submissions. "</td><td>". $c_submission->id;
	echo "</td><td>".$c_submission->vpl."</td><td>".count($y_data)."</td><td>".$s."</td><td>".$c_submission->datesubmitted;
	echo "</td></tr>";
