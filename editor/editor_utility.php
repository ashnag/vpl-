...
class vpl_editor_util {
    public static function generate_jquery() {
         ...
         */
        {
            ...
            $PAGE->requires->js( new moodle_url( '/mod/vpl/editor/jquery/jquery-2.1.4.min.js' ));
    ...
    public static function generate_requires() {
        ...
        $PAGE->requires->js( new moodle_url( '/mod/vpl/editor/lz-string.min.js' ) );
        $PAGE->requires->js( new moodle_url( '/mod/vpl/editor/term.js' ) );
        $PAGE->requires->css( new moodle_url( '/mod/vpl/editor/VPLIDE.css' ) );
        $PAGE->requires->js( new moodle_url( '/mod/vpl/editor/VPLTerminal.js' ) );
        $PAGE->requires->js( new moodle_url( '/mod/vpl/editor/aceModification.js' ) );
...
<script>
    ...
    $JQVPL(document).ready(function(){
        ...
        rsFunctionsObject.joptions=<?php echo $joptions;?>;
        rsFunctionsObject.userid=<?php 
        $userids = optional_param('userid',FALSE,PARAM_INT);
        echo '"'.$userids.'"'?>;
    });
    </script>
<?php
