	...
	if (typeof VPL_IDE == 'undefined') {
		...
		VPL_IDE = function(root_id, options) {
			...
			var VPL_File = function(name, value) {
				...
				//Code Added
				startLogging(fileName,editor,$JQVPL("#vpl_file" + id),options);
				autoSave(fileName,editor,$JQVPL("#vpl_file" + id));
				//Code added
			...
			rsFunctionsObject.getFiles=getFiles;
			var readOnly = false;
			...
			var myId = options['id'];
			var myUserId = options['userid'];
			function showTimer(message, options) {
				var message_dialog = $JQVPL('<div class="vpl_ide_dialog"></div>');
				if (typeof options == 'undefined') {
					options = {};
				}
				...
				if (typeof options.ok == 'undefined') {
					message_buttons[str('ok')] = function() {
						$JQVPL(this).dialog('close');
						window.location= 'edit.php?id='+ myId +'&userid='+myUserId;
					};
				} else {
					message_buttons[str('ok')] = function() {
						$JQVPL(this).dialog('close');
			...
			rsFunctionsObject.autoSaveFunc = function(){
				var timer = null,minutes,seconds;

				function display(mssg_dialog){
					...
					mssg_dialog.setMessage("Time Left: " + theTime);
					timer--;
					if(timer==0){
						...
						mssg_dialog.dialog('close');
						window.location= '../view.php?id='+ myId;
					}
						setTimeout(function () {
							display(mssg_dialog);
						}, 1000);
				...
				function startTimer() {
					timer = 60;
					var mssg = showTimer("Time Left: " + timer, {
			...
			menuActions['save'] = function() {
                                if(rsFunctionsObject.files !== undefined){
                                    for(var i=0;i<rsFunctionsObject.files.length;i++){
	                                    rsFunctionsObject.files[i]();
                                    }
                                }
				requestAction('save', 'saving', getFiles(), function() {
					global_modified = false;
			...
			rsFunctionsObject.rsSaveFunction=menuActions['save'];
			...
			if(rsFunctionsObject.runFunc !== undefined){
								console.log("running");
					rsFunctionsObject.runFunc();
				}
