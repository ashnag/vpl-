...
class mod_vpl_submission {
    ...
    function checkout($revision,$filepath){

        if(file_exists($filepath)) {
            $string = exec("rlog " . $filepath . "|grep head");
            $rev = (int)substr($string, 6);
            if ($rev >= (int)$this->instance->id) {
                system("co -l -f" . $revision . " " . $filepath);
            } else {
                system("co -l -f " . $filepath);
            }
        }

    }

    function checkoutForRead($revision,$filepath){
        ...
        if(file_exists($filepath)) {
            ...
            if ($rev >= (int)$this->instance->id) {
                ...
                system("rcs -u ". $filepath);
        }

    }

    function getRevisionMessage($filePath,$revision){
        $string = exec("rlog " .$filePath ." |grep -A2 \"revision ". $revision. "\" |head -n 3");
        return $string;
    }

    function setRevisionMessage($filePath,$revision,$message){
        $string = exec("rcs -m".$revision.":".$message ." ".$filePath);
    }

    function checkin($revision,$message,$description,$filepath){
        system("ci -u" . $revision . " -f -m'" . $message . "' -t-'" . $description . "' " . $filepath);
    }
    /**
     * @return object file group manager for submitted files
     **/
    function get_submitted_fgm_for_update(){
        ...
        $fgm= $this->submitted_fgm;

        $this->checkout($this->instance->id,$this->get_submissionfilelistname());
        ...
        for($i = 0; $i < $nf; $i ++) {
            ...
            $location = $this->get_submission_directory() . $filename;
            $this->checkout($this->instance->id,$location);
        }
    ...
    function get_submitted_fgm(){
        ...
        $this->checkoutForRead($this->instance->id,$this->get_submissionfilelistname());
        ...
        for($i = 0; $i < $nf; $i ++) {
            ...
            $filename = str_replace('/','=',$filen);
            $location = $this->get_submission_directory() . $filename;
            $this->checkoutForRead($this->instance->id,$location);
        }
        return $this->submitted_fgm;
    }
     ...
     * @return string file name
     **/
    function get_gradecommentsfilename_for_update(){
        $this->checkout($this->instance->id,$this->get_data_directory().'grade_comments.txt');
        return $this->get_data_directory().'grade_comments.txt';
    }

    function get_gradecommentsfilename(){
        $this->checkoutForRead($this->instance->id,$this->get_data_directory().'grade_comments.txt');
        return $this->get_data_directory().'grade_comments.txt';
    }
    ...
    function set_submitted_file($files){
        ...
        $currRev = $this->instance->id;
        $message = "Revision ".$currRev;
        $description = "Submission";


        $fg=$this->get_submitted_fgm_for_update();

        foreach($files as $file){
            $filename = str_replace('/','=',$file['name']);
            $location = $this->get_submission_directory() . $filename;
            $fg->addFile($file['name'],$file['data']);
            $this->checkin($currRev,$message,$description,$location);
        }
        $this->checkin($currRev,$message,$description,$this->get_submissionfilelistname());
    ...
    function remove_grade(){
        ...
        if(!$DB->update_record('vpl_submissions',$this->instance)){
            ...
            if(file_exists($fn) && ($this->getRevisionMessage($fn,$this->instance->id)!= "Deleted" ) && ($this->getRevisionMessage($fn,$this->instance->id)!= null )){
//                unlink($fn);
                $this->setRevisionMessage($fn,$this->instance->id,"Deleted");
            }
        }
        return true;
    }
    ...
    function set_grade($info, $automatic=false){
        ...
        if($scaleid!=0){
            ...
            $fn = $this->get_gradecommentsfilename_for_update();
            if($comments){
                $fp = vpl_fopen($fn);
                fwrite($fp,$comments);
                fclose($fp);
                $this->checkin($this->instance->id,"Revision ".$this->instance->id,"Grade Submission",$fn);
            } else if(file_exists($fn) && ($this->getRevisionMessage($fn,$this->instance->id)!= "Deleted" ) && ($this->getRevisionMessage($fn,$this->instance->id)!= null )){
//                unlink($fn);
                $this->setRevisionMessage($fn,$this->instance->id,"Deleted");
            }
            //update gradebook
    ...
    function saveCE($result){
        ...
        $this->checkout($this->instance->id,$compfn);
        if(file_exists($compfn) && ($this->getRevisionMessage($compfn,$this->instance->id)!= "Deleted" ) && ($this->getRevisionMessage($compfn,$this->instance->id)!= null )){
//                unlink($fn);
            $this->setRevisionMessage($compfn,$this->instance->id,"Deleted");
        }
        $execfn = $this->get_data_directory().'/'.self::executionFN;
        $this->checkout($this->instance->id,$execfn);
        if(file_exists($execfn) && ($this->getRevisionMessage($execfn,$this->instance->id)!= "Deleted" ) && ($this->getRevisionMessage($execfn,$this->instance->id)!= null )){
//                unlink($fn);
            $this->setRevisionMessage($execfn,$this->instance->id,"Deleted");
        }
        file_put_contents($compfn,$result['compilation']);
        if($result['executed']>0){
            file_put_contents($execfn,$result['execution']);
        }
        $this->checkin($this->instance->id,"Revision ".$this->instance->id,"Compilation Submission",$compfn);
        $this->checkin($this->instance->id,"Revision ".$this->instance->id,"Execution Submission",$execfn);
    ...
    function getCE(){
        $ret = array();
        $compfn = $this->get_data_directory().'/'.self::compilationFN;
        $this->checkoutForRead($this->instance->id,$compfn);
        if(file_exists($compfn) && ($this->getRevisionMessage($compfn,$this->instance->id)!= "Deleted" ) && ($this->getRevisionMessage($compfn,$this->instance->id)!= null ) ){
        ...
        $execfn = $this->get_data_directory().'/'.self::executionFN;
        $this->checkoutForRead($this->instance->id,$execfn);
        if(file_exists($execfn) && ($this->getRevisionMessage($execfn,$this->instance->id)!= "Deleted" ) && ($this->getRevisionMessage($execfn,$this->instance->id)!= null )){
            $ret['executed']=1;
