...
class mod_vpl_submission_CE extends mod_vpl_submission{
    ...
    function prepare_execution($type, &$already=array(), $vpl=null){
        ...
        if($type == 2){ //If evaluation add information
            ...
            $info .='export MOODLE_USER_ID='.($subinstance->userid)."\n"; 
            //----------------------------------
            $user_ROHIT = get_complete_user_data('id', $subinstance->userid);
            //$user_ROHIT = get_field('user', 'username', 'id', $subinstance->userid);
			//-----------------------------------
            $info .='export MOODLE_USER_ID_NAME='.($user_ROHIT->username)."\n";
